...
const connectPackageJson = JSON.parse(
  fs.readFileSync("apps/connect/package.json", "utf8"),
);
const connectVersion = connectPackageJson.version;

function getVersionFromProjectsVersionData(
  ...
  for (const [packageName, packageData] of Object.entries(
    ...
    if (version !== null && packageVersion && packageVersion !== version) {
      throw new Error(
        `Version mismatch for ${packageName}: ${version} !== ${packageVersion}`,
      );
    }
  }
...
async function getPreReleaseResults(specifier?: string, preid?: string) {
  ...
  const isUsingCurrentVersion = Object.values(projectsVersionData).some(
    (project) => project.newVersion === project.currentVersion,
  );

  if (isUsingCurrentVersion) {
    console.warn(
      "You're using the current version, no changes will be released",
    );
    result.isUsingCurrentVersion = true;
  }

  result.isUsingOlderVersion = Object.values(projectsVersionData).some(
    (project) => {
  ...
  const version =
    workspaceVersion ||
    getVersionFromProjectsVersionData(
  ...
  if (
    changelogResult.workspaceChangelog?.contents.length &&
    changelogResult.workspaceChangelog?.contents.length >
      MAX_RELEASE_CONTENTS_LENGTH
  ) {
    result.isChangelogTooLong = true;
  }

  result.isEmptyRelease =
    !!changelogResult.workspaceChangelog?.contents.includes(
      "This was a version bump only, there were no code changes",
    );

  const matchTag =
    changelogResult.workspaceChangelog?.releaseVersion.rawVersion.match(
      /v?\d+\.\d+\.\d+-([a-zA-Z]+)\.\d+/,
    );
...
(async () => {
  ...
  if (version && ["patch", "minor", "major"].includes(version) && !tag) {
    ...
    if (semverObject && semverObject.prerelease.length > 0) {
      ...
      specifier = `${newVersion}-${prerelease}.1`;
      preid = prerelease;
    }
  }
  ...
  if (isBranchRelease) {
    ...
    specifier = preid
      ? `${normalizedBranchVersion}-${preid}.1`
      : normalizedBranchVersion;
  ...
  if (preReleaseResult.isUsingOlderVersion) {
    console.error(
      ">>> The version calculated for the release is older thatn the current version, check that there are no missing tags",
    );
    process.exit(1);
  }
  ...
  if (preReleaseResult.isChangelogTooLong) {
    console.warn(
      `>>> Release changelog is too long, limiting to last ${MAX_COMMITS_PER_RELEASE} commits`,
    );
    to = TO;
    from = FROM;
  }
