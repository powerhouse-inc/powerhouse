...
const buildExampleDocument = ():ScopeFrameworkObject => {
    ...
    framework.updateElementComponents({ 
        id:scopeId, 
        components: {
            content: 'The Governance Scope covers principles and rules that regulate the critical balance of power, and adjudicate on appeals processes related to misalignment in the ecosystem. The Governance Scope must ensure that the resilient equilibrium of SkyDAO Governance remains protected against all potential direct and indirect threats.'
        } 
    });
    ...
    framework.addElement({ 
        ...
        components: {
            content: 'Governance Nebulae manage the Scope Advisors, from soliciting proposals to ensuring the produced work aligns with the Scope Artifact. They also have the authority to propose changes to the Scope Framework in response to ambiguous or challenging situations.'
        }
    });
    ...
    framework.addElement({ 
        ...
        components: {
            content: 'The Spirit of the Atlas represents the foundational principles of SkyDAO Governance, enshrined in the Immutable Documents and reflected in Atlas Documents and aligned participants of the Sky Ecosystem.'
        }
    });
...
describe('ScopeFramework Class', () => {
    ...
    it('should create an empty document', () => {
        ...
        expect(framework.state.elements[0].id).toMatch(/^[a-zA-Z0-9+\\/]{27}=$/);
    ...
    it('should apply main operations', () => {
        ...
        expect(framework.state.elements.filter(e => e.path == 'B.1.3').length).toBe(1);
        
        const elementId = framework.state.elements.filter(e => e.path == 'B.1.3')[0].id;
        ...
        framework.state.elements.filter(e => e.id == elementId).forEach(e => {
        ...
        const removeId = framework.state.elements.filter(e => e.path == 'B.1.2')[0]?.id;        
        framework.removeElement({id: removeId});
        expect(framework.state.elements.filter(e => e.path.startsWith('B.1.2')).length).toBe(0);
    });
