...
describe('skip operations', () => {
    describe('skip operation param', () => {
        ...
        it('should include skip param in base operations with provided value', () => {
            let document = createDocument();
            document = emptyReducer(document, setName('TEST_1'), undefined, { skip: 1, ignoreSkipOperations: true });
            document = emptyReducer(document, setName('TEST_2'), undefined, { skip: 2, ignoreSkipOperations: true });
            document = emptyReducer(document, setName('TEST_3'), undefined, { skip: 3, ignoreSkipOperations: true });
    ...
    describe('replayOperations', () => {
        it('should ignore operation 2, when operation 3 -> (skip=1)', () => {
            ...
            document = countReducer(                            // valid operation, skip 1
                document,
                increment(),
                undefined,
        ...
        it('should ignore operation 2, 3 and 4, when operation 5 -> (skip=3)', () => {
            ...
            document = countReducer(document, increment());     // skipped
            document = countReducer(document, increment());     // skipped
            document = countReducer(                            // valid operation, skip 3
                document,
                increment(),
                undefined,
        ...
        it('should ignore all the previous operations, when operation 5 -> (skip=4)', () => {
            ...
            document = countReducer(document, increment());     // skipped
            document = countReducer(document, increment());     // skipped
            document = countReducer(                            // valid operation, skip 4
                document,
                increment(),
                undefined,
        ...
        it('should skip operations when dispatch a new action with an skip value', () => {
            ...
            expect(document.operations.global[0]).toHaveProperty('type', 'INCREMENT');
            expect(document.operations.global[1]).toHaveProperty('type', 'NOOP');
            expect(document.operations.global[2]).toHaveProperty('type', 'INCREMENT');
            expect(document.operations.global[3]).toHaveProperty('type', 'NOOP');
            expect(document.operations.global[4]).toHaveProperty('type', 'INCREMENT');
        });
    });
});
