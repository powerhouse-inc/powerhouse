...
import { profileOperation } from "./helpers/profiling.js";
...
// Set timeout to 60 seconds for profiled tests (profiling adds overhead)
...
test("Create ToDoDocument Model", async ({ page }) => {
  ...
  // Create and setup document (profiled)
  await profileOperation("setupDocument", async () => {
  ...
  // Verify all document aspects (profiled)
  await profileOperation("verifyDocumentBasicData", async () => {
    await verifyDocumentBasicData(page, TEST_DOCUMENT_DATA);
  });

  await verifyDocumentInitialState(page);

  await profileOperation("verifyDocumentOperations", async () => {
    await verifyDocumentOperations(page, TEST_DOCUMENT_DATA);
  });

  await verifyDocumentOperationHistory(page);

  // Export and validate document (profiled)
  await profileOperation("exportAndValidateDocument", async () => {
...
test("Create a TodoList", async ({ page }) => {
  ...
  // Profile the todo document creation
  await profileOperation("createTodoDocument", async () => {
...
async function setupDocument(
  ...
  await goToConnectDrive(page, "My Local Drive");
  // Profile the document creation and data filling
  await profileOperation("createDocumentAndFillBasicData", async () => {
