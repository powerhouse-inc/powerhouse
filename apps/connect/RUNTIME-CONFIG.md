...
- **`connect.pwa` (the project's own block) is build-time only.** `ph connect build` merges it (together with build-time package `pwa` fragments) into the base `manifest.webmanifest` and the injectManifest `service-worker.js` — including the manifest's `file_handlers` (the built-in `.phd`/`.phdm` OS file association plus any contributed types), `launch_handler`, and the extended members (`shortcuts`, `protocol_handlers`, `share_target`, `screenshots`, `categories`, `display_override`). It is carried along in the dist file for transparency, but the SPA never reads the project block — editing the dist copy or injecting it via `PH_CONNECT_CONFIG_JSON` has no effect. See the Academy page "Configure the PWA" for the merge semantics.
- **Packages installed AT RUNTIME dynamically extend the manifest.** The service worker serves `manifest.webmanifest` dynamically, merging the build-embedded base with the `pwa` fragments of packages installed at runtime (mirrored into IndexedDB `ph-pwa` by `BrowserPackageManager`; scalars are base-wins so a package can add but not rename/re-theme, and `.phd`/`.phdm` stay first-registered). Note the browser/OS caveat: `file_handlers`/`protocol_handlers`/`launch_handler` are captured at PWA install time, so a runtime-added file/protocol association becomes discoverable only via Chrome's periodic manifest-update check (best-effort, browser-dependent) — never synchronously for the running installed session. Opening a runtime package's `.phd`/`.phdm` documents already works via content-based routing regardless.
