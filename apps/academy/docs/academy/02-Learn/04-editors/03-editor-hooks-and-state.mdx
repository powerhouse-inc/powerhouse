...
You've written an editor shell. Now you need to actually _read_ the document and _change_ it. Powerhouse gives you a small set of hooks from `@powerhousedao/reactor-browser` to do exactly that — no manual subscriptions, no wiring.
...
Always generate IDs _before_ calling dispatch — reducers must stay pure.
...
<Quiz
  ...
  choices={[
    {
      label:
        "useDocument(id) — it fetches the document and I can call dispatch on the result.",
      explanation:
        "useDocument(id) is Suspense-based and returns the document only — no dispatch. It's for reading a document by ID, not for the editor's primary action dispatch.",
    },
    {
      label:
        "useSelectedTodoListDocument() — it returns [doc, dispatch], and dispatch sends the action.",
      correct: true,
      explanation:
        "Exactly right. The generated typed hook returns a tuple: the document and a dispatch function. Call dispatch(completeTodo({ id })) inside the button's onClick.",
    },
    {
      label:
        "useSelectedDocument() — it's the most generic option, so it always works.",
      explanation:
        "It works, but you lose type safety. Prefer the generated typed hook when you know the document model.",
    },
    {
      label:
        "No hook needed — I can import dispatch directly from the Reactor.",
      explanation:
        "The Reactor is not imported directly in editor components. The hooks are the supported interface and handle subscriptions for you.",
    },
  ]}
/>
