  ...
  choices={[
    {
      ...
      explanation:
        "Rendering is the editor's job. Reducers are pure functions that never touch the DOM.",
    },
    {
      label: "Showing a 'Saved!' toast after the dispatch succeeds — reducer.",
      explanation:
        "Toast notifications are a UI side-effect. That belongs in the editor, not in a pure function.",
    },
    {
      label:
        "Computing the next document state when the action arrives — reducer.",
      correct: true,
      explanation:
        "State transitions are exactly what reducers do. The editor dispatches the action; the reducer takes it from there.",
    },
    {
      label: "Validating that the new title is not empty — editor.",
      explanation:
        "Input validation that determines whether a state transition is valid belongs in the reducer, so it runs consistently regardless of which editor triggered the action.",
    },
  ]}
/>
