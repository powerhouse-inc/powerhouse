...
Notice what the editor does _not_ do:
...
<Quiz
  ...
  choices={[
    {
      label:
        "Editor mutates document.state.global.items[i].done → UI re-renders.",
      explanation: "Editors must never mutate state. They dispatch actions.",
    },
    {
      label:
        "Editor dispatches completeTodo → reducer produces next state → Reactor appends operation → UI re-renders with new state.",
      correct: true,
      explanation:
        "The action flows through the reducer and is recorded by the Reactor; the new state comes back to the editor.",
    },
    {
      label:
        "Editor fetches /api/todos/complete → server updates DB → UI re-renders.",
      explanation:
        "That's CRUD. Powerhouse inverts it: the log is local first, and syncs itself later.",
    },
    {
      label:
        "Editor calls completeTodo() directly without dispatch, so it runs synchronously.",
      explanation:
        "Actions are just data. The dispatch step is what turns them into operations via the reducer.",
    },
  ]}
...
<Cards>
  <Card
    ...
    href="/academy/Learn/editors/editor-hooks-and-state"
  />
  <Card
    title="Back to Your first document model"
    href="/academy/Learn/document-models/your-first-document-model"
  />
</Cards>
