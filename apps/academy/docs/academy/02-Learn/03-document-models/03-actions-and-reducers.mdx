  ...
  choices={[
    {
      ...
      explanation: "Random fallbacks make the reducer non-deterministic — two replays produce different IDs. Generate the ID in the action input instead.",
    },
    {
      label: "state.createdAt = new Date().toISOString()",
      explanation: "Wall-clock time is non-deterministic. Peers replaying this operation at different moments get different timestamps. Pass the timestamp in the action input.",
    },
    {
      label: "const user = await fetch('/me').then(r => r.json()); state.owner = user.id",
      explanation: "Reducers must be synchronous and have no I/O. A network call makes the reducer async and unreliable. The owner ID should be included in the action input.",
    },
    {
      label: "const t = state.todos.find(x => x.id === action.input.id); if (!t) throw new TodoNotFoundError('not found'); t.done = !t.done;",
      correct: true,
      explanation: "Pure, synchronous, deterministic — this is exactly what a valid reducer looks like. Same state + same action always produces the same result.",
    },
  ]}
/>
...
<Cards>
  <Card title="Your first document model" href="/academy/Learn/document-models/your-first-document-model" />
  <Card title="Back to: State and scopes" href="/academy/Learn/document-models/state-and-scopes" />
</Cards>
