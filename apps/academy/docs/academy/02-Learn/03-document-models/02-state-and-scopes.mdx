  ...
  choices={[
    {
      ...
      explanation: "Presence indicators are a UI concern, not document state. Unposted drafts are private by nature — sharing them globally would be noisy and unexpected.",
    },
    {
      label: "local — it's private to this user's device until they post.",
      correct: true,
      explanation: "Draft text that hasn't been submitted is a perfect local-scope use case: it only matters on this device, and should never sync.",
    },
    {
      label: "Neither — draft text shouldn't be stored in document state at all.",
      explanation: "Storing ephemeral input in local state is exactly the right pattern. It gives you persistence across refreshes without polluting the shared log.",
    },
    {
      label: "global, but only after a short delay.",
      explanation: "Delaying the sync doesn't change the fact that unposted text is private. Use local scope and only move content to global when the user posts.",
    },
  ]}
/>
...
<Cards>
  <Card title="Actions and reducers" href="/academy/Learn/document-models/actions-and-reducers" />
  <Card title="Back to: Documents and drives" href="/academy/Learn/document-models/documents-and-drives" />
</Cards>
