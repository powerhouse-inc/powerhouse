...
- Node.js ≥ 24
- The `ph` CLI (`npm i -g @powerhousedao/ph-cmd` or see [Installation](/academy/Build/GettingStartedBuilding/Prerequisites))
...
<Term>Powerhouse</Term> wraps reducers in [Mutative](https://mutative.js.org/),
so you write direct mutations — no need to return a new object. The framework
produces an immutable snapshot from each mutation.
  ...
  choices={[
    {
      ...
      correct: true,
      explanation:
        "Network calls are side-effects — they belong in a processor or in the Reactor, not the reducer.",
    },
    {
      label: "Reading the current time with Date.now() to timestamp the todo.",
      correct: true,
      explanation:
        "Anything non-deterministic (time, randomness, environment) breaks replay. Timestamps come from the operation metadata.",
    },
    {
      label: "Filtering items with Array.prototype.filter.",
      explanation:
        "Pure. filter() returns a new array and touches nothing outside the reducer.",
    },
    {
      label: "Writing state.items.push(newItem) to add the item.",
      explanation:
        "This is the correct Powerhouse pattern. Mutative wraps the reducer so mutations produce an immutable snapshot — push and splice are both fine.",
    },
  ]}
