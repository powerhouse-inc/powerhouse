...
For your model's own operations you never call `createAction` directly: use the generated creator (`setCursor(...)`) from `document-models/<your-model>/gen/creators.ts`. The exception is the built-in `document`-scope actions (`CREATE_DOCUMENT`, `UPGRADE_DOCUMENT`, `DELETE_DOCUMENT`), which have no generated creator. `@powerhousedao/reactor` exports `createDocumentAction`, `upgradeDocumentAction` and `deleteDocumentAction` for them, and outside a reactor you call `createAction` yourself with `"document"` as the fifth argument.
