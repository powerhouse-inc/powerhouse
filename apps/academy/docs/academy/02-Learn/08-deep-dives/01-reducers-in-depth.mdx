...
For your model's own operations, never call `createAction` directly. Use the generated creator (`setCursor(...)`) from `document-models/<your-model>/gen/creators.ts`. The built-in `document`-scope actions (`CREATE_DOCUMENT`, `UPGRADE_DOCUMENT`, `DELETE_DOCUMENT`) have no generated creator. `@powerhousedao/reactor` exports `createDocumentAction`, `upgradeDocumentAction` and `deleteDocumentAction` for them. Outside a reactor, call `createAction` yourself with `"document"` as the fifth argument.
