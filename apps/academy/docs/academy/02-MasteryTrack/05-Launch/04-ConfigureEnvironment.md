...
## Build-time env-var seeds

These environment variables remain supported as build-time **seed** values (set-if-absent). They are the legacy interface and are intended for CI / docker workflows that pre-date the JSON file. Setting the same field in `powerhouse.config.json` (or via a `--flag` or `--json` override) always wins.

```bash
# connect.app
PH_CONNECT_BASE_PATH="/"           # also consumed by nginx for routing
PH_CONNECT_LOG_LEVEL="info"

# connect.packages
PH_CONNECT_EXTERNAL_PACKAGES_DISABLED="false"   # inverted → externalEnabled
PH_CONNECT_PACKAGES_REGISTRY="https://registry.dev.vetra.io"

# connect.drives
PH_CONNECT_DISABLE_ADD_DRIVE="false"            # inverted → allowAddDrive
PH_CONNECT_DEFAULT_DRIVES_URL=""                # comma-separated URLs → defaultDrives[]
PH_CONNECT_DRIVES_PRESERVE_STRATEGY=""

# connect.drives.sections.remote (PUBLIC env-var names are the legacy alias)
PH_CONNECT_PUBLIC_DRIVES_ENABLED="true"
PH_CONNECT_DISABLE_ADD_PUBLIC_DRIVES="false"    # inverted → allowAdd
PH_CONNECT_DISABLE_DELETE_PUBLIC_DRIVES="false" # inverted → allowDelete
...
In a Docker deployment, the entrypoint also seeds these into the dist file at container start (set-if-absent), so operators can keep using `docker run -e PH_CONNECT_RENOWN_URL=…` patterns without changing anything.
