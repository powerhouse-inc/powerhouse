...
This page covers the low-level `IReactor` interface and the internal components you can access through `ReactorModule`. Most developers should use `IReactorClient` (see [IReactorClient API Reference](/academy/Reference/APIReferences/ReactorClient)) — the information here is for advanced scenarios such as:
