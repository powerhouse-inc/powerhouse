...
Let's walk through the typical workflow for using a component from the document-engineering system, using `BooleanField` for a checkbox in the [TodoList editor](/academy/Build/BuildingUserExperiences/BuildingDocumentEditors).
...
You can learn how to do so in our guide on [Creating Custom Scalars](/academy/Reference/EditorsUI/CreateCustomScalars).
