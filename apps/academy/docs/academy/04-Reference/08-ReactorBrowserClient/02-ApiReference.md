...
the endpoint has to change. `attachmentService` is the exception: it is
published from its own effect, so an app that builds it asynchronously can pass
it later without throwing away every open document.
