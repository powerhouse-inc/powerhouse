...
const action = createAction<UpgradeDocumentAction>(
  "UPGRADE_DOCUMENT",
  {
    model: todoUpgradeManifest.documentType,
    fromVersion,
    toVersion,
...
// Runs each transition's upgradeReducer in order, then stamps toVersion
const upgraded = applyUpgradeDocumentAction(document, action, path);
```

`applyUpgradeDocumentAction` runs each transition in `path` and stamps `state.document.version` on the document it returns. It does not clone: the version is written in place, so the document you passed in is the one that changed. Clone first (`structuredClone(document)`) if the caller needs the original. It rewrites `initialState` only when the action input carries an `initialState` (or `state`) field; the `createAction` call above carries neither, so stamping `initialState.document.version` is the caller's job, and each `upgradeReducer` must migrate `initialState` alongside `state` or a later replay fails its hash check. It appends nothing to the operation log. The reactor records the upgrade as one `UPGRADE_DOCUMENT` operation in the `document` scope. To mirror the reactor, build the operation with `operationFromAction(action, index, skip, context)`. `index` is the next index in `operations.document`, `skip` is `0`, and `context` is an `OperationContext` (`{ documentId, documentType, scope: "document", branch, ordinal }`).
