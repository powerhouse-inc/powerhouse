...
`download()` is the one asymmetric case: it resolves as soon as the stream is available, with `downloading` as the last event and `loaded: 0`. Byte events and the terminal `done` arrive as the caller reads the stream, and never arrive if the caller abandons it — so `downloadMany`'s `concurrency` bounds target negotiation rather than simultaneous transfer. Batch progress is per item, stamped with `index`, alongside an `onItemSettled` callback carrying `{ settled, completed, failed, total }`.
