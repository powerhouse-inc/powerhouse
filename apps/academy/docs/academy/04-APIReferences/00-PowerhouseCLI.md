...
Command Overview:
  ...
  It starts a local development server with a Vetra Switchboard, enabling real-time document
  model editing and processing with a "Vetra" drive instead of "Powerhouse".

  This command:
  1. Starts a local Vetra development server
  2. Sets up a Vetra Switchboard for document processing
  3. Enables real-time updates and code generation
  4. Configures necessary services for Vetra development
...
Options:
  ...
  -w, --watch              Watch for local changes to document models and processors,
                          and automatically update the Vetra Switchboard accordingly.

Examples:
  $ ph vetra                                           # Start Vetra dev environment with defaults
  $ ph vetra --generate                                # Auto-generate code on model changes
  $ ph vetra --switchboard-port 5000                   # Use custom port for Vetra Switchboard
