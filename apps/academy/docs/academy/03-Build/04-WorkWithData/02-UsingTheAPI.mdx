...
Once you've structured and captured data from your business processes, use Switchboard to build experiences in external websites, create interactive drive widgets, or generate reports and dashboards in Fusion.
...
GraphQL SDL is the backbone of **Specification Driven Design & Development** at Powerhouse. By defining your document models in SDL, you create machine-readable specifications that developers, designers, and AI agents all work from.
...
**Fetching data from the Reactor:** Powerhouse uses GraphQL to expose system-level data, such as drives, users, and operational records through the **Reactor Subgraph**, which allows querying of drives, stored files and folders.
...
Use a **GraphQL** query to read a document model. This continues with the **To-do List example** from the [introduction tutorial](/academy/Build/ManualTodoTutorial/CreateNewPowerhouseProject), but the process applies to any document model.
To make our document model available in the Apollo Studio Sandbox, we'll need to store it on a remote [Reactor](/academy/Reference/Reactor/WorkingWithTheReactor).
...
Click the (+) 'Create New Drive' to add a public drive. To add a new drive, you'll have to know the correct public URL of the drive. Read more about [configuring drives](/academy/Reference/Reactor/WorkingWithTheReactor).
...
With the remote reactor and/or drive running, store a document model on it.
Create a new to-do list document in Connect Studio named **'Powerhouse-onboarding-tasks'**.
...
When you navigate to your Switchboard endpoint by adding **`/graphql`** to the end of your URL (e.g., http://localhost:4001/graphql, https://switchboard.phd/graphql, or add it to a custom domain), you can use this document ID to query the state of your document.
...
e.g., http://localhost:4001/graphql, https://switchboard.phd/graphql, or your custom https://switchboard.domain/graphql
...
This example query is structured to request all documents of type `ToDoList`.
It extracts common metadata fields such as **id**, **name**, **documentType**, **revisionsList**, **createdAtUtcIso**, and **lastModifiedAtUtcIso**.
...
The previous step queried for document metadata. Now query for the actual content of the document state.
...
{
  "identifier": "03eb6780-f1d7-438c-84a0-6d93dfb8f6af"
}
```
...
Add a new item to the list. The document model for the to-do list has an `ADD_TODO_ITEM` operation, which translates to an `addTodoItem` mutation in GraphQL.
...
Replace the example `docId` with the actual ID of your document. You can get this ID by querying as we did before.

When you execute this mutation in Apollo Studio, it will add the new item to your to-do list. The response will return the updated document with its new revision.
...
mutation DeleteTodoItem($docId: PHID!, $input: ToDoList_DeleteTodoItemInput!) {
  ToDoList {
    deleteTodoItem(docId: $docId, input: $input) {
      id
      name
      revisionsList {
        scope
...
1.  **Query the document state again:** Rerun the `document` query from earlier in this tutorial. You should see the new item in the list or the deleted item removed.
...
You can now read and write documents over the GraphQL API. To expose your own queries and mutations on top of this data, see [Using Subgraphs](/academy/Build/WorkWithData/UsingSubgraphs).
