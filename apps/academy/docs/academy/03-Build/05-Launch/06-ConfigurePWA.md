...
`connect.pwa.manifest` overrides the [web app manifest](https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps/Manifest). Scalars deep-merge over the defaults and your values win; [`icons`](https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps/Manifest/Reference/icons) and `file_handlers` append to the built-in set.
...
| `file_handlers`                    | opens `.phd` and `.phdm` files — see [File handling](#file-handling) |
...
The installed PWA registers as an **OS-level handler** for Powerhouse document files through the manifest's [`file_handlers`](https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps/Manifest/Reference/file_handlers) member: opening a `.phd` or `.phdm` file from the file system launches Connect, which asks which drive to import into and imports the file(s) there. A single opened file is then opened in its editor; when several files are opened at once they are imported together and each gets an "Open document" link in the upload panel. The manifest also sets [`launch_handler`](https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps/Manifest/Reference/launch_handler) to `focus-existing`, so an already-running Connect window is focused and receives the files instead of a new window opening per file. Inside the app, the files arrive through the [`LaunchQueue`](https://developer.mozilla.org/en-US/docs/Web/API/LaunchQueue) API.

Browser and OS support for file handling varies — check the compatibility tables on the MDN pages above. Where a platform does not support it, the association is simply never registered and Connect keeps working normally. Each handler entry may declare per-type `icons` for the file association; whether and how the OS displays them is also platform-dependent, and some platforms derive document icons from the app icon instead.

A package or the project can associate **additional file types** by contributing `file_handlers` entries:
...
- Entries only declare **which** file types to accept: `accept`, optional `icons` and optional `launch_type`. There is **no `action` field** — opening the files requires Connect's own runtime handling, so every handler opens at Connect's app root. A fragment that tries to set `action` fails validation.
- Extensions must start with a dot. This is validated at build time, since browsers silently ignore entries without it.
- Contributed entries are **appended after** Connect's built-in handler and de-duplicated on the whole entry. Browsers associate each extension with the first handler that claims it, so the built-in `.phd` and `.phdm` association cannot be overridden by a package.
- File handling only activates for an **installed** PWA on a production build — it cannot be exercised through `ph connect dev`.
...
| `icons`, `file_handlers`, globs, `runtimeCaching`, denylist | **Additive**                   |
