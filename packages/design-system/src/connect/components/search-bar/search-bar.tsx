...
export const ConnectSearchBar: React.FC<ConnectSearchBarProps> = (props) => {
  ...
  return (
    <div className={cn("flex items-center", className)}>
      ...
      <input
        className={twMerge(
          "flex h-[52px] min-w-0 flex-1 items-center rounded-xl border border-gray-200 bg-gray-50 px-4 text-sm text-slate-200 outline-none dark:border-slate-500 dark:text-slate-700 dark:bg-slate-600",
        )}
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent className="rounded-xl border border-gray-100 bg-gray-50 p-2 dark:border-slate-500 dark:bg-slate-600">
          {items.map((item) => (
            <DropdownMenuItem
