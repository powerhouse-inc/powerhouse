      ...
      <div className="flex w-full shrink-0 justify-between">
        <List className="flex w-full gap-x-2 rounded-xl p-1 text-sm font-semibold text-gray-700 outline-none dark:text-slate-200">
          {React.Children.map(children, (child, _i) => {
            ...
            return (
              <Trigger
                className="flex min-h-7 flex-1 items-center justify-center rounded-lg border border-gray-300 bg-gray-50 py-2 text-gray-900 transition duration-300 data-disabled:cursor-not-allowed data-disabled:text-gray-400 data-[state=active]:bg-gray-100 data-[state=active]:text-gray-900 dark:border-slate-500 dark:bg-slate-600 dark:text-slate-200 dark:data-disabled:text-slate-500 dark:data-[state=active]:bg-slate-500 dark:data-[state=active]:text-slate-100"
                key={label as string}
