...
describe("Budget Statement Audit Report reducer", () => {
  ...
  it.skip("should add audit report", async () => {
    ...
    expect(updatedDocument.state.global.auditReports[0]).toStrictEqual(input);
    expect(document.state.global.auditReports).toStrictEqual([]);
  ...
  it("should set default timestamp on audit report", async () => {
    ...
    expect(
      newDocument.state.global.auditReports[0].timestamp >= date.toISOString(),
    ).toBe(true);
  });
  ...
  it("should add needs action audit report", async () => {
    ...
    expect(document.state.global.auditReports[0].status).toBe("NeedsAction");
  });
  ...
  it("should save attachment to zip", async () => {
    ...
    expect(JSON.parse(attachmentZip?.comment ?? "")).toStrictEqual(attributes);
  });
  ...
  it("should load attachment from zip", async () => {
    ...
    expect(document.state.global.auditReports[0].status).toBe("NeedsAction");
    expect(
      (document.operations.global[0] as Operation<AddAuditReportAction>).input
        .report,
    ).toBe(hash);
