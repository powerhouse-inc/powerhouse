...
export interface EditorStateProps {
  ...
  readonly latestOperation?: Operation<DocumentModelAction | BaseAction> | null;
}
...
function EditorState(props: EditorStateProps) {
  ...
  return (
    ...
    >
      ...
      >
        <h4 style={{ marginBottom: "16px" }}>{`${scopeName} State Schema`}</h4>
        <EditorSchema
          height={200}
          name={name}
          onGenerate={(schema) => {
      ...
      >
        <h4 style={{ marginBottom: "16px" }}>{`${scopeName} Initial State`}</h4>
        <EditorInitialState
          height={200}
          onCreate={(value) => {
