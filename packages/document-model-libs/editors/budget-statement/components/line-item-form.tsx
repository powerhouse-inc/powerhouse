...
const LineItemForm: React.FC<{
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    addLineItem({
      ...
      category: categories.find((c) => c.id === formJson.category.toString())!,
  ...
  return (
    <form
      key={
        accounts.find((a) => a.address === selectedAccount)?.lineItems.length
      }
      method="post"
    ...
    >
      ...
      <pre />
      <label>
        Budget Cap: <input name="budgetCap" placeholder="0" type="number" />
      </label>
      <pre />
      <label>
