...
export const codeFirstDocumentEditorFileTemplate = (v: {
  documentModelId: string;
}) =>
  tsx`
import { DocumentStateViewer, DocumentToolbar } from "@powerhousedao/design-system/connect";
import { useSelectedDocumentOfType } from "@powerhousedao/reactor-browser";
import { baseActions, type Action, type PHDocument } from "document-model";

export default function Editor() {
  const [document, dispatch] = useSelectedDocumentOfType<PHDocument, Action>(
    "${v.documentModelId}",
  );

  const handleSetName = (name: string) => {
    dispatch(baseActions.setName(name));
  };
