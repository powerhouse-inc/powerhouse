...
export function makeUpgradeManifestsExport({
  ...
  const outputSourceFile = project.getSourceFile(absoluteOutputFilePath);
  if (!outputSourceFile) {
    return;
  }
  ...
  const manifests = upgradeManifestFiles.map((file) => {
    ...
    const aliasedName = `${documentModelName}UpgradeManifest`;
  ...
  // Add import declarations for each upgrade manifest
  const manifestImports = manifests.map(
    ({ originalName, aliasedName, moduleSpecifier }) => ({
      namedImports: [`${originalName} as ${aliasedName}`],
      moduleSpecifier,
    }),
  );
  outputSourceFile.addImportDeclarations(manifestImports);
