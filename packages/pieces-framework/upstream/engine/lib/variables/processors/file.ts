// Vendored from activepieces/activepieces@0.91.0 packages/server/engine/src/lib/variables/processors/file.ts. MIT; see ../../../../../LICENSE.
// Generated by scripts/sync-upstream.mts — do not edit by hand.
import { Readable } from "node:stream";
import { isBase64, isNil, isString } from "../../../../core-utils/index.js";
import type { ApStreamingFile } from "../../../../framework/index.js";
import { ApFile, PropertyType } from "../../../../framework/index.js";
...
export const fileProcessor: ProcessorFn = async (property, urlOrBase64) => {
  ...
  const streaming =
    property.type === PropertyType.FILE && property.streaming === true;
  try {
    if (streaming) {
...
): { extension: string; buffer: Buffer } | null {
...
async function handleStreamingFile(
  propertyValue: string,
): Promise<ApStreamingFile | null> {
  const parsed = parseBase64File(propertyValue);
  if (!isNil(parsed)) {
    return {
  ...
  const sizeIsTrustworthy =
    (isNil(contentEncoding) || contentEncoding.toLowerCase() === "identity") &&
