// Vendored from activepieces/activepieces@0.91.0 packages/pieces/framework/src/lib/piece.ts. MIT; see ../../../LICENSE.
...
import type { PieceBase, PieceMetadata } from "./piece-metadata.js";
...
> implements Omit<PieceBase, "version" | "name"> {
  ...
  public getContextInfo: (() => { version: ContextVersion }) | undefined =
    () => ({ version: LATEST_CONTEXT_VERSION });
  constructor(
    ...
    if (
      !isValidSimpleSemver(minimumSupportedRelease) ||
      isSemverLessThan(
...
function withConnectionIdentifierFlag(
  ...
  return Array.isArray(auth)
    ? auth.map(flagConnectionIdentifier)
    : flagConnectionIdentifier(auth);
}

function flagConnectionIdentifier(auth: PieceAuthProperty): PieceAuthProperty {
  return {
    ...auth,
    hasConnectionIdentifier: auth.getConnectionIdentifier !== undefined,
  };
}
