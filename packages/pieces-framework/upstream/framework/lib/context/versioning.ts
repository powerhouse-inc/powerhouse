// Vendored from activepieces/activepieces@0.91.0 packages/pieces/framework/src/lib/context/versioning.ts. MIT; see ../../../../LICENSE.
// Generated by scripts/sync-upstream.mts — do not edit by hand.
import { PauseType } from "../../../core-piece-types/index.js";
import type { ActionContext, PauseHook } from "./index.js";
import type { InputPropertyMap, PieceAuthProperty } from "../property/index.js";
...
export const backwardCompatabilityContextUtils = {
  makeActionContextBackwardCompatible({
    context,
    contextVersion,
  }: MakeActionContextBackwardCompatibleParams): ActionContext<
    PieceAuthProperty,
    InputPropertyMap
  > {
...
function addLegacyMethodsAndServerUrl({
  context,
}: {
  context: ActionContext<PieceAuthProperty, InputPropertyMap>;
}): ActionContext<PieceAuthProperty, InputPropertyMap> {
...
function buildLegacyPauseHook({
  ...
  return (req) => {
    const type =
      req.pauseMetadata.type === PauseType.DELAY
        ? ("DELAY" as const)
        : ("WEBHOOK" as const);
    const responseToSend =
      req.pauseMetadata.type === PauseType.WEBHOOK
        ? req.pauseMetadata.response
        : undefined;
    const resumeDateTime =
      req.pauseMetadata.type === PauseType.DELAY
        ? req.pauseMetadata.resumeDateTime
        : undefined;
    context.run
      .createWaitpoint({ type, version: "V0", resumeDateTime, responseToSend })
      .catch((e) => {
 ...
 * @deprecated Since 2026-04-12. Remove after 2026-10-12 once all pieces migrate to createWaitpoint/waitForWaitpoint.
 */
function buildLegacyGenerateResumeUrl({
  context,
}: {
  context: ActionContext<PieceAuthProperty, InputPropertyMap>;
}): (params: {
  ...
  return (params) => {
    ...
    const url = new URL(
      `${context.server.publicUrl}v1/flow-runs/${context.run.id}/requests/${randomId}${params.sync ? "/sync" : ""}`,
    );
