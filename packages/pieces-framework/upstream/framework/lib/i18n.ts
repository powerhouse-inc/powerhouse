// Vendored from activepieces/activepieces@0.91.0 packages/pieces/framework/src/lib/i18n.ts. MIT; see ../../../LICENSE.
...
import { MAX_KEY_LENGTH_FOR_CORWDIN } from "../../core-piece-types/index.js";
import path from "node:path";
import fs from "node:fs/promises";

export const pieceTranslation = {
  translatePiece: <T extends PieceMetadataModelSummary | PieceMetadataModel>(
    params: TranslatePieceParams<T>,
  ): T => {
    ...
    try {
      ...
      const translatedPiece: T = mutate
        ? piece
...
function translateProperty(
  pieceModelOrProperty: Record<string, unknown>,
  path: string,
  i18n: Record<string, string>,
) {
  const parsedKeys = path.split(".");
  if (parsedKeys[0] === "*") {
    return Object.values(pieceModelOrProperty).forEach((item) =>
      translateProperty(
        item as Record<string, unknown>,
        parsedKeys.slice(1).join("."),
        i18n,
      ),
    );
  }
  const nextObject = pieceModelOrProperty[parsedKeys[0]] as Record<
    string,
    unknown
  >;
  if (!nextObject) {
...
const readLocaleFile = async (locale: LocalesEnum, pieceOutputPath: string) => {
  ...
  try {
    ...
    throw new Error(
      `Invalid i18n file format for ${locale} in piece ${pieceOutputPath}`,
    );
  } catch (error) {
    console.error(
      `Error reading i18n file for ${locale} in piece ${pieceOutputPath}:`,
      error,
    );
    return null;
  }
...
type TranslatePieceParams<
  T extends PieceMetadataModelSummary | PieceMetadataModel,
> = {
  piece: T;
  locale?: LocalesEnum;
  mutate?: boolean;
