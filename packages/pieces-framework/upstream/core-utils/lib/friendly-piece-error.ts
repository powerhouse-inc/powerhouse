// Vendored from activepieces/activepieces@0.91.0 packages/core/utils/src/lib/friendly-piece-error.ts. MIT; see ../../../LICENSE.
// Generated by scripts/sync-upstream.mts — do not edit by hand.
import { tryCatchSync } from "./try-catch.js";
...
const HTML_TAG_LIKE_REGEX =
  /<\s*(html|body|head|title|meta|p|div|span|h[1-6]|table|script|style)\b/i;
...
const stripMarkup = (value: string): string => {
  ...
  const textOnly = decodeHtmlEntities(withoutNoise.replace(/<[^>]+>/g, " "))
    .replace(/\s+/g, " ")
    .trim();
...
const collectMessageFrom = ({
  value,
  depth,
  seen,
}: TraversalParams): string | undefined => {
  ...
  if (Array.isArray(value)) {
    ...
    const parts = value
      .map((entry) =>
        collectMessageFrom({ value: entry, depth: depth + 1, seen }),
      )
...
const extractResponseHttpDetails = (
  ...
  const requestUrl =
    request === undefined ? undefined : readString(request, "url");
  const requestMethod =
    request === undefined ? undefined : readString(request, "method");
...
const extractClientHttpDetails = (
  error: Record<string, unknown>,
): HttpDetails | null => {
...
const extractHttpDetails = (
  error: Record<string, unknown>,
): HttpDetails | null => {
  return extractResponseHttpDetails(error) ?? extractClientHttpDetails(error);
};

const rebuildSerializable = ({
  value,
  depth,
  seen,
}: TraversalParams): unknown => {
  if (
    isNil(value) ||
    isString(value) ||
    typeof value === "number" ||
  ...
  if (Array.isArray(value) || isObjectRecord(value)) {
    ...
    if (Array.isArray(value)) {
      return value.map((entry) =>
        rebuildSerializable({ value: entry, depth: depth + 1, seen }),
      );
    }
...
const toSerializable = (value: unknown): unknown => {
  ...
  const rebuilt = tryCatchSync(() =>
    rebuildSerializable({ value, depth: 0, seen: new WeakSet() }),
  );
  return isNil(rebuilt.error) ? rebuilt.data : UNSERIALIZABLE_PLACEHOLDER;
};

const toSerializableHttpDetails = (
  ...
  return {
    ...
    responseHeaders: isNil(responseHeaders)
      ? undefined
      : Object.fromEntries(
...
const isFriendlyPieceError = (value: unknown): value is FriendlyPieceError => {
  ...
  return (
    value["__apErrorVersion"] === FRIENDLY_PIECE_ERROR_VERSION &&
    typeof value["message"] === "string"
  );
};

export const formatPieceError = (
  error: unknown,
  options?: FormatPieceErrorOptions,
): FriendlyPieceError => {
...
export const tryParseFriendlyPieceError = (
  value: unknown,
