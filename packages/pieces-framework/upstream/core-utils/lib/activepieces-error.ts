// Vendored from activepieces/activepieces@0.91.0 packages/core/utils/src/lib/activepieces-error.ts. MIT; see ../../../LICENSE.
...
export type PieceBundleNotAvailableParams = BaseErrorParams<
  ErrorCode.PIECE_BUNDLE_NOT_AVAILABLE,
  {
    pieceName: string;
    pieceVersion: string;
    status: number;
...
export type NoChatResponseParams = BaseErrorParams<
  ErrorCode.NO_CHAT_RESPONSE,
  Record<string, never>
>;

export type ChatContextLimitExceededParams = BaseErrorParams<
...
export type ChatMessageLimitExceededParams = BaseErrorParams<
  ErrorCode.CHAT_MESSAGE_LIMIT_EXCEEDED,
  {
    limit: number;
...
export type PausedFlowTimeoutExceededParams = BaseErrorParams<
  ErrorCode.PAUSED_FLOW_TIMEOUT_EXCEEDED,
  {
...
export type FlowExternalIdAlreadyExistsParams = BaseErrorParams<
  ErrorCode.FLOW_EXTERNAL_ID_ALREADY_EXISTS,
  {
    externalId: string;
  }
>;
...
export type SandboxCapacityExceededParams = BaseErrorParams<
  ErrorCode.SANDBOX_CAPACITY_EXCEEDED,
  Record<string, never>
>;
