// Vendored from activepieces/activepieces@0.91.0 packages/core/utils/src/lib/ai-provider-health.ts. MIT; see ../../../LICENSE.
...
export function toProviderOutcomeSignal(error: unknown): ProviderOutcomeSignal {
  ...
  return {
    ...
    ...(formatted.responseBody === undefined
      ? {}
      : { body: stringifyBody(formatted.responseBody) }),
...
export function observedProviderFetch(
  onOutcome: ProviderOutcomeReporter | undefined,
  ...
  return async (input, init) => {
    ...
    try {
      ...
      report({
        observed: Promise.resolve(toProviderOutcomeSignal(error)),
        onOutcome,
      });
      throw error;
    }
...
function report({
  ...
  observed: Promise<ProviderOutcomeSignal>;
...
async function readEnoughToClassify(
  response: Response,
...
export function classifyProviderOutcome({
  ...
  body,
  message,
}: ProviderOutcomeSignal): AiProviderKeyStatus | NoStatusChange {
  if (!isNil(statusCode)) {
    return classifyByStatus({
      statusCode,
      haystack: `${body ?? ""} ${message ?? ""}`,
    });
  }
  const text = message ?? "";
...
function classifyByStatus({
  ...
  statusCode: number;
  haystack: string;
}): AiProviderKeyStatus | NoStatusChange {
...
const TRANSIENT_ERROR_PATTERN =
  /\b(429|5\d\d)\b|rate.?limit|timeout|timed out|temporarily|try again|econnreset|etimedout|socket hang up|service unavailable/i;

const BILLING_BODY_PATTERN =
  /insufficient_quota|credit[_ ]balance|billing_hard_limit_reached|billing|\bcredits?\b|out of funds|payment required/i;

const RATE_LIMIT_BODY_PATTERN =
  /per minute|per day|per_minute|per_day|requests? per|tokens? per|rate.?limit|resource_exhausted|\brpm\b|\btpm\b/i;

const MODEL_NOT_FOUND_PATTERN = /model|deployment|engine/i;

export const AiProviderKeyStatus = z.enum([
  "active",
  "out_of_credits",
  "rejected",
  "unreachable",
...
export type ProviderOutcomeReporter = (
