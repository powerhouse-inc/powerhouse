// Vendored from activepieces/activepieces@0.91.0 packages/server/engine/test/variables/file-processor.test.ts. MIT; see ../../../../../LICENSE.
...
import { propsProcessor } from "../../../../../upstream/engine/lib/variables/props-processor.js";

const HELLO_TXT_DATA_URL = "data:text/plain;base64,aGVsbG8=";
const FILE_URL = "https://example.com/report.csv";

async function resolveStreamingFile(
  input: unknown,
  required = true,
): Promise<{
...
describe("File Processor", () => {
  ...
  it("resolves a streaming URL input to a lazy body, deriving size and name from headers", async () => {
    ...
    const file = processedInput.file as ApStreamingFile;
  ...
  it("resolves a buffered URL input to null when the URL responds with a non-ok status", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(
        new Response("<Error>AuthenticationFailed</Error>", {
          status: 403,
