// Vendored from activepieces/activepieces@0.91.0 packages/core/piece-types/src/lib/ai-providers.test.ts. MIT; see ../../../../../LICENSE.
// Generated by scripts/sync-upstream.mts — do not edit by hand.
import {
  AIProviderName,
  isNil,
} from "../../../../../upstream/core-utils/index.js";
...
import {
  ...
  MANAGED_MODEL_WEIGHTS,
  MODELS_AWAITING_A_CREDIT_WEIGHT,
} from "../../../../../upstream/core-piece-types/lib/ai-providers.js";
...
describe("AI_PROVIDER_CAPABILITIES", () => {
  ...
  it("marks embedding support iff a default embedding model exists", () => {
    for (const provider of Object.values(AIProviderName)) {
      ...
      expect(caps.supportsEmbedding).toBe(
        caps.defaultEmbeddingModel !== undefined,
      );
    }
  });
});
...
describe("getCuratedChatModels", () => {
  it("is undefined for providers that pick a model through chat tiers", () => {
    expect(
      aiProviderUtils.getCuratedChatModels({
  ...
  it("never returns an empty list, so callers can treat a result as non-empty", () => {
    for (const provider of Object.values(AIProviderName)) {
      expect(
        aiProviderUtils.getCuratedChatModels({ provider })?.length ?? 1,
      ).toBeGreaterThan(0);
    }
  });
});
...
describe("isCuratedChatModelId", () => {
  it("accepts every tier id and every curated model id", () => {
    for (const tier of ACTIVEPIECES_CHAT_TIERS) {
      expect(aiProviderUtils.isCuratedChatModelId({ modelId: tier.id })).toBe(
        true,
      );
    }
    for (const curatedIds of Object.values(ALLOWED_CHAT_MODELS_BY_PROVIDER)) {
      for (const id of curatedIds) {
        expect(aiProviderUtils.isCuratedChatModelId({ modelId: id })).toBe(
          true,
        );
      }
    }
  ...
  it("accepts the model every tier actually runs on, so bumping a tier cannot strand it outside the vocabulary", () => {
    for (const tier of ACTIVEPIECES_CHAT_TIERS) {
      expect(
        aiProviderUtils.isCuratedChatModelId({ modelId: tier.modelId }),
        tier.modelId,
      ).toBe(true);
    }
  });
  ...
  it("rejects anything outside that vocabulary", () => {
    expect(aiProviderUtils.isCuratedChatModelId({ modelId: "gpt-9" })).toBe(
      false,
    );
...
describe("managed chat model vocabulary", () => {
  it("covers every tier model, so tier drift can never deny the model a tier runs on", () => {
    for (const tier of ACTIVEPIECES_CHAT_TIERS) {
      expect(
        aiProviderUtils.isManagedChatModelId({ modelId: tier.modelId }),
        tier.modelId,
      ).toBe(true);
    }
  });
  ...
  it("covers every id the managed allow-list declares", () => {
    for (const id of ALLOWED_CHAT_MODELS_BY_PROVIDER[
      AIProviderName.ACTIVEPIECES
    ] ?? []) {
      expect(aiProviderUtils.isManagedChatModelId({ modelId: id }), id).toBe(
        true,
      );
    }
  });
  ...
  it("rejects an empty or arbitrary string", () => {
    ...
    expect(
      aiProviderUtils.isManagedChatModelId({ modelId: "anything/at-all" }),
    ).toBe(false);
  });
...
describe("canDisableReasoning", () => {
  it("is true for every tier model, so the default path keeps its zero-reasoning first step", () => {
    for (const tier of ACTIVEPIECES_CHAT_TIERS) {
      expect(
        aiProviderUtils.canDisableReasoning({ modelId: tier.modelId }),
        tier.modelId,
      ).toBe(true);
    }
  });
  ...
  it("is false for the reasoning-native models that rejected a disable directive in production", () => {
    for (const id of [
      ...
      expect(aiProviderUtils.canDisableReasoning({ modelId: id }), id).toBe(
        false,
      );
    }
  });
  ...
  it("is false for a managed model we have never observed accepting one", () => {
    expect(
      aiProviderUtils.canDisableReasoning({
        modelId: "google/gemini-3.7-flash",
      }),
    ).toBe(false);
    expect(
      aiProviderUtils.canDisableReasoning({ modelId: "x-ai/grok-4.20" }),
    ).toBe(false);
  });
});
...
describe("tier native model ids", () => {
  it("names a model the native Anthropic list actually offers, so no tier resolves to an id that does not exist", () => {
    for (const tier of ACTIVEPIECES_CHAT_TIERS) {
      expect(
        ALLOWED_CHAT_MODELS_BY_PROVIDER[AIProviderName.ANTHROPIC],
        tier.id,
      ).toContain(tier.nativeModelId);
    }
  });
...
describe("every managed model we offer has a credit weight somebody chose", () => {
  ...
  const priced = (model: string) =>
    ACTIVEPIECES_CHAT_TIERS.some((tier) => tier.modelId === model) ||
    !isNil(MANAGED_MODEL_WEIGHTS[model]);

  it("offers nothing that quietly falls back to the default rate, apart from what is still pending", () => {
    expect(offered.filter((model) => !priced(model))).toEqual(
      MODELS_AWAITING_A_CREDIT_WEIGHT,
    );
  });
