// Vendored from activepieces/activepieces@0.91.0 packages/core/utils/test/ai-provider-health.test.ts. MIT; see ../../../../LICENSE.
// Generated by scripts/sync-upstream.mts — do not edit by hand.
import { describe, expect, it, vi } from "vitest";
import type { ProviderOutcomeSignal } from "../../../../upstream/core-utils/lib/ai-provider-health.js";
import {
  classifyProviderOutcome,
  observedProviderFetch,
} from "../../../../upstream/core-utils/lib/ai-provider-health.js";
...
describe("classifyProviderOutcome", () => {
  ...
  it("leaves the status alone for a bad request, which is the caller's fault", () => {
    expect(
      classifyProviderOutcome({ statusCode: 400, body: "invalid temperature" }),
    ).toBe("no_change");
    expect(
      classifyProviderOutcome({ statusCode: 422, body: "unprocessable" }),
    ).toBe("no_change");
  });
  ...
  describe("without a status code, falling back to the message", () => {
    it("still catches credit exhaustion", () => {
      expect(
        classifyProviderOutcome({ message: "You are out of credits" }),
      ).toBe("out_of_credits");
      expect(
        classifyProviderOutcome({ message: "HTTP 402 payment required" }),
      ).toBe("out_of_credits");
    });

    it("holds the status for a transient failure", () => {
      expect(
        classifyProviderOutcome({ message: "429 rate limit exceeded" }),
      ).toBe("no_change");
      expect(classifyProviderOutcome({ message: "socket hang up" })).toBe(
        "no_change",
      );
      expect(classifyProviderOutcome({ message: "ETIMEDOUT" })).toBe(
        "no_change",
      );
    });
...
describe("observedProviderFetch", () => {
  const withFetch = async (
    ...
    try {
      const observed = observedProviderFetch((signal) => {
        signals.push(signal);
      });
  ...
  it("caps how much of a failure body it keeps", async () => {
    const signals = await withFetch(
      async () => new Response("x".repeat(5000), { status: 429 }),
    );
  ...
  it("reports a transport failure and rethrows it untouched", async () => {
    ...
    try {
      ...
      await expect(observed?.("https://api.openai.com/v1/models")).rejects.toBe(
        boom,
      );
      await new Promise((resolve) => setImmediate(resolve));
    } finally {
