// Vendored from activepieces/activepieces@0.91.0 packages/core/utils/test/friendly-piece-error.test.ts. MIT; see ../../../../LICENSE.
// Generated by scripts/sync-upstream.mts — do not edit by hand.
import { describe, expect, it } from "vitest";
import { formatPieceError } from "../../../../upstream/core-utils/lib/friendly-piece-error.js";
...
describe("formatPieceError api message extraction", () => {
  ...
  it("reads an array of plain strings", () => {
    const formatted = formatPieceError(
      httpError({ status: 422, responseBody: ["name is required"] }),
    );
  ...
  it("leaves apiMessage unset when an array body carries no message", () => {
    const formatted = formatPieceError(
      httpError({ status: 500, responseBody: [{ code: 42 }] }),
    );
...
describe("formatPieceError on cyclic and unbounded error bodies", () => {
  it("terminates on a self referencing array body", () => {
    ...
    const formatted = formatPieceError(
      httpError({ status: 500, responseBody: body }),
    );
  ...
  it("still reads the message that sits above a cycle", () => {
    ...
    const formatted = formatPieceError(
      httpError({ status: 401, responseBody: body }),
    );
  ...
  it("collects a shared reference once, the cost of being cycle safe", () => {
    ...
    const formatted = formatPieceError(
      httpError({ status: 422, responseBody: [entry, entry] }),
    );
...
describe("formatPieceError never throws while formatting", () => {
  it("keeps a serializable body byte identical rather than rewriting it", () => {
    const responseBody = {
      at: new Date("2020-01-02T03:04:05Z"),
      blob: Buffer.from("hi"),
      nested: [1, "two", { three: true }],
    };

    const formatted = formatPieceError(
      httpError({ status: 400, responseBody }),
    );

    expect(JSON.stringify(formatted.responseBody)).toBe(
  ...
  it("survives a body whose own getter throws", () => {
    ...
    Object.defineProperty(responseBody, "boom", {
      enumerable: true,
      get: () => {
    ...
    const formatted = formatPieceError(
      httpError({ status: 401, responseBody }),
    );
  ...
  it("survives a bigint that plain stringify refuses", () => {
    const formatted = formatPieceError(
      httpError({ status: 500, responseBody: { count: BigInt(9) } }),
    );
