...
const unsubscribeJobCompleted = jobExecutor.on('jobCompleted', (result: JobResult) => {
  console.log(`Job ${result.job.id} completed in ${result.duration}ms`);
});

const unsubscribeJobFailed = jobExecutor.on('jobFailed', (result: JobResult) => {
...
console.log(`Success rate: ${status.totalJobsSucceeded}/${status.totalJobsProcessed}`);
