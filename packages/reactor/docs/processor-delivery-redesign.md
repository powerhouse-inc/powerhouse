...
- **Dedupe without a window.** There is no overlap set. After each backfill page is delivered, the queue strips those ordinals from the `live` tasks queued behind it: routing is synchronous, so any delivered ordinal the manager has already routed to this processor is in the queue at that point. Ordinals above the routing high-water (`max(lastOrdinal, highest routed)`) have not been routed at all; they go into a small `unrouted` set and are dropped once when their live batch arrives. What remains: a live op below the high-water that reaches the manager after the backfill read it is delivered twice, which at-least-once allows. That happens whenever another document's pass has already raised the high-water past it, not only through the §7 Postgres visibility gap; telling the two apart would need the manager to know which indexed ops have not reached it yet.
...
- **Re-registration waits for the previous instance.** `unregisterFactory` still resolves without waiting for disconnects, but a later `registerFactory` under the same identifier runs its factory only after the previous processors' queues have drained and `onDisconnect` has run, and after any call of the previous factory still in flight has returned and had its records discarded, so two instances never share a namespace. Batches routed meanwhile are covered by the new processors' backfill through the pending slot.
