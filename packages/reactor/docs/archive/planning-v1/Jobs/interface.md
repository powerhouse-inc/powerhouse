...
/**
 * Identifies the furthest write-side index that must be observed for a document,
 * scope, and branch tuple.
 */
...
/**
 * Returned with every completed job so clients can enforce read-after-write semantics.
 */
...
/**
 * Persisted job metadata exposed to clients and internal trackers.
 */
export type JobInfo = {
  id: string;
  status: JobStatus;
...
## Consistency Token Lifecycle

- Executors return `OperationWithContext` payloads that include `documentId`,
  `scope`, and `branch` information for every generated operation.
- The executor manager normalizes those payloads into a single
  `ConsistencyToken` by keeping the highest operation index per tuple and
  stamping the current timestamp.
- `IJobTracker.markCompleted` requires the token, ensuring every `JobInfo` that
  reaches clients carries it, even when no operations were emitted (empty
  coordinate list).
- Callers supply the token to read APIs which, in turn, delegate to the shared
  consistency tracker and the read models’ `waitForConsistency` methods before
  fetching data.
