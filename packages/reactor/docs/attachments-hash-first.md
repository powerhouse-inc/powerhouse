...
- **Dedup oracle -- closed by default.** `reserve()` rejection (and `stat()` on an arbitrary hash) reveals whether given content exists on the server. This oracle already exists today -- any client can `stat()` any hash -- and hash-first makes probing cheaper and more natural. Therefore `POST /attachments/reservations` **requires upload-equivalent authorization by default**: probing for content is treated as privileged as adding content. `GET`/`HEAD /attachments/:hash` go further and answer only a caller who can read the attachment through a named document (or holds a signed URL minted under one), with the same `404` for a refusal and an unknown hash, so they are no oracle at all. Transports carry their credentials and the `documentId` on fetch. The 409 body's metadata surface stays minimal by design (ref only -- no file name, no size).
