...
describe("runWorker in-process execution", () => {
  ...
  it("defers a job whose trust policy asks for a later retry, keeping the delay", async () => {
    const h = await startInProcessWorker((spec) => {
      if (spec.module.exportName === "createTrustPolicy") {
        return Promise.resolve({
          authorizeSigner() {
            const error = new Error("no credential yet") as Error & {
              retryAfterMs: number;
            };
    ...
    const job: Job = {
      ...
      maxRetries: 0,
      errorHistory: [],
      meta: { batchId: "batch-defer-1", batchJobIds: ["job-defer-1"] },
    };
    ...
    h.port1.postMessage({
      type: "execute",
      correlationId: "corr-defer-1",
      job,
    });

    const result = await resultPromise;
    expect(result.result.success).toBe(false);
    expect(result.error?.name).toBe("DeferredAdmissionError");
    expect(result.error?.retryAfterMs).toBe(250);
    expect(result.error?.cause?.name).toBe("MissingCredentialError");
  });
