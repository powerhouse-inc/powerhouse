...
describe("InMemoryQueue", () => {
  ...
  describe("retryJobAfter", () => {
    ...
    it("holds the job at the head of its stream until the delay elapses", async () => {
      vi.useFakeTimers();
      try {
        const first = createTestJob({ id: "first", maxRetries: 3 });
        const second = createTestJob({ id: "second" });
        await queue.enqueue(first);
        await queue.enqueue(second);

        const handle = await queue.dequeueNext();
        expect(handle?.job.id).toBe("first");
        mockEventBusEmit.mockClear();
        await queue.retryJobAfter("first", 100, error);

        expect(await queue.dequeueNext()).toBeNull();
        expect(await queue.dequeue("doc-1", "global", "main")).toBeNull();
        ...
        expect(retried?.job.id).toBe("first");
        expect(retried?.job.retryCount).toBe(0);
        expect(retried?.job.deferral?.count).toBe(1);
        expect(retried?.job.errorHistory).toEqual([error]);
        await queue.completeJob("first");
        expect((await queue.dequeueNext())?.job.id).toBe("second");
      } finally {
        vi.useRealTimers();
      }
    });

    it("keeps the first deferral's time across deferrals", async () => {
      vi.useFakeTimers();
      try {
        await queue.enqueue(createTestJob({ id: "job" }));
        await queue.dequeueNext();
        await queue.retryJobAfter("job", 10, error);
        const firstAtMs = (queue as InMemoryQueue).getJob("job")!.deferral!
          .firstAtMs;

        await vi.advanceTimersByTimeAsync(10);
        await queue.dequeueNext();
        await queue.retryJobAfter("job", 20, error);

        expect((queue as InMemoryQueue).getJob("job")!.deferral).toEqual({
          firstAtMs,
          count: 2,
        });
      } finally {
    ...
    it("lets other streams run while one is held", async () => {
      await queue.enqueue(createTestJob({ id: "held" }));
      await queue.enqueue(createTestJob({ id: "other", documentId: "doc-2" }));
      await queue.dequeueNext();
      await queue.retryJobAfter("held", 60_000, error);

      expect((await queue.dequeueNext())?.job.id).toBe("other");
      expect(queue.isDrained).toBe(false);
      await queue.clearAll();
    });

    it("rejects while the queue is blocked", async () => {
      await queue.enqueue(createTestJob({ id: "job" }));
      await queue.dequeueNext();
      queue.block();

      await expect(queue.retryJobAfter("job", 10, error)).rejects.toThrow(
        "Queue is blocked",
      );
    });
  });
