...
describe("Tests the Reactor with the Document Drive Document Model", () => {
  ...
  async function waitForJobAndDocumentUpdate(jobId: string): Promise<void> {
    await vi.waitUntil(
      async () => {
  ...
  describe("ADD Operations", () => {
    ...
    it("should add a file via reactor.mutate", async () => {
      ...
      const folderJobInfo = await reactor.execute(document.header.id, "main", [
        folderAction,
      ]);

      // Wait for job completion and document view update

      await waitForJobAndDocumentUpdate(folderJobInfo.id);
      ...
      await waitForJobAndDocumentUpdate(fileJobInfo.id);
  ...
  describe("UPDATE Operations", () => {
    it("should update a file via reactor.mutate", async () => {
      ...
      const updateJobInfo = await reactor.execute(document.header.id, "main", [
        updateAction,
      ]);
  ...
  describe("DELETE Operations", () => {
    it("should delete a node via reactor.mutate", async () => {
      ...
      const deleteJobInfo = await reactor.execute(document.header.id, "main", [
        deleteAction,
      ]);
    ...
    it("should delete children when parent is deleted", async () => {
      ...
      await waitForJobAndDocumentUpdate(setupJobInfo.id);
  ...
  describe("Document Query Operations", () => {
    ...
    describe("find() - Search by Parent ID", () => {
      it("should find children of a parent document", async () => {
        ...
        await waitForJobAndDocumentUpdate(relJobInfo.id);
