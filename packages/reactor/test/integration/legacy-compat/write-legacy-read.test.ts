...
const prismaClient = new PrismaClient({
  datasources: {
    db: {
      url: "postgresql://postgres:postgres@localhost:5400/postgres",
    },
  },
});
...
const storageLayers = [
  ...
  //["PrismaStorage", async () => new PrismaStorage(prismaClient as any, cache)],
] as unknown as [
  string,
...
/**
 * These tests validate that when writing through the Reactor, reading through
 * the legacy storage or through the document view results in the same document.
 */
  ...
  (storageName, buildStorage) => {
    ...
    describe("reactor.mutate() validation", () => {
      it("should maintain equivalent documents in both stores after multiple mutations", async () => {
        ...
        const viewFile = viewDoc.state.global.nodes.find(
          (n) => n.id === fileId,
        );
