...
- `IWriteCache` stores only the last operation per scope (the write-cache slicing contract). UNDO, REDO, and NOOP+skip all need the full history, so the executor invalidates the cache for those action types before loading the document. NOOP+skip arises during sync reshuffling in `executeLoadJob`.
