    ...
    Mutation: {
      registerPullResponderListener: async (
        ...
        if (this.drivePermissionService) {
          ...
          const isSyncable =
            await this.drivePermissionService.isSyncable(driveId);
          if (!isSyncable) {
            this.logger.warn(
              `registerPullResponderListener rejected: drive ${driveId} is PRIVATE and not syncable`,
            );
          ...
          const canRead = await this.drivePermissionService.canRead(
            driveId,
            ctx.user?.address,
          );
          ...
          if (!canRead) {
            this.logger.warn(
              `registerPullResponderListener rejected: user ${ctx.user?.address ?? "anonymous"} lacks read permission for drive ${driveId}`,
            );
            throw new GraphQLError(
      ...
      pushUpdates: async (
        ...
        if (this.drivePermissionService) {
          ...
          const canWrite = await this.drivePermissionService.canWrite(
            driveId,
            ctx.user?.address,
          );
