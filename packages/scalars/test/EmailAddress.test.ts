...
import { EmailAddressScalar } from "../src/scalars/EmailAddress.js";

describe("EmailAddress Scalar", () => {
  it("should serialize an email address", () => {
    const email = "test@test.com";

    expect(EmailAddressScalar.scalar.serialize(email)).toBe(email);
  });

  it("should throw an error if the value is not a string", () => {
    const email = 123;

    expect(() => EmailAddressScalar.scalar.serialize(email)).toThrow();
  });
  ...
  it("should parse a valid email address", () => {
    ...
    expect(EmailAddressScalar.scalar.parseValue(email)).toBe(email);
  });

  it("should throw an error if parse a value that is not a valid email address", () => {
    const email = "test";

    expect(() => EmailAddressScalar.scalar.parseValue(email)).toThrow();
  });
  ...
  it("should parse a valid email address from a literal", () => {
    ...
    expect(
      EmailAddressScalar.scalar.parseLiteral({
        kind: Kind.STRING,
        value: email,
      }),
    ).toBe(email);
