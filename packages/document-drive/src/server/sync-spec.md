...
_Early exploration of the pull request user flow between document drives_
...
_CQRS-inspired architecture plan for Switchboard_
...
- A **synchronization thread** is a synchronization channel that propagates changes _from one document instance to another._ It can be comprised of multiple strands.
  ...
  - A **cross-document synchronization thread** is a thread with `doc_id_1 != doc_id_2`. This is a more unusual case, and a thread should be explicitly called _cross-document thread_ if it applies. Cross-document threads may sometimes come in handy for copying documents or creating new documents based on a pre-defined template document.
- A **synchronization cable** is a synchronization channel with one or more threads _between two drive instances._ A synchronization cable may contain multiple threads.
  `(drive_url_1, ?, ?, ?) -> (drive_url_2, ?, ?, ?)`
...
- A **synchronization thread** between two document instances will typically not propagate all scopes and/or branches.
  - If it does synchronize _all scopes and branches_, we call this a **complete synchronization thread**.
...
- A **synchronization cable** between two drive instances will typically not propagate all documents with all their scopes and branches in the entire drive.
  - If it does synchronize _all documents, scopes, and branches,_ we call this a **complete synchronization cable.**
...
- For example: a file storage adapter _may_ derive drive ID and document ID from its folder structure if it consistently stores documents in `<drive>/<doc_id>.ext.zip` files. Further parameters may be extracted from the document files itself.
...
- The `ListenerFilter` structure needs to be further refined, but this is where we indicate which documents, scopes, and branches the listener is interested in.
  ...
  - `scope` can be left undefined to set up _complete_ threads or cables, but should typically be set to `[public]` or `[public, protected]` for synchronization of Public Drives or Secure Cloud Drives.
...
_Listeners settings page with a user-defined webhook selected (details may change)_
...
_Listener 2 is up-to-date for sync_id 53. Listener 1 is not._
...
2. For each selected listener L, for each selected synchronization unit S,
   if the filter `L.filter(S)` is a match, add a new record to the **Listener State Cache** with an assumed listener revision number of 0**:** - listener_id = L.listenerId - block = L.block - listener_rev = 0 - listener_status = `CREATED` - pending_timeout = _null_ - sync_rev = S.revision - sync_id = S.id
...
We distinguish between both aspects as defined below. The notation we’re using puts a _caron_ on the side where the initial trigger is located:
...
- **Webhooks `Š ↦ R`**
  _“Our Connect Drive is pushing pre-processed changes to an external Receiver.”_
- **Data processing service (for example BA Labs) `S ↦ Ř` (or `S ⇥ Ř`)**
  _“An external Receiver is pulling pre-processed (or unprocessed) changes from our Switchboard Sender.”_
- **Download public drive updates (Connect ← Switchboard) `Ř ⇤ S`**
  _“Our Connect Drive is pulling unprocessed changes from a Switchboard Sender.”_
- **Push changes to a Public Drive or Secure Cloud Drive `Š ⇥ R`**
  _“Our Connect Drive is pushing unprocessed changes to a Switchboard Receiver.”_
...
- RWA reporting example: _An error could be shown when the assets, which name should be adjusted, have already been deleted._
...
2. **[Receiver] REQUEST (only in case of a Pull Update)**
   ...
   ⇒ Ideally, more robust pull signals may ask the sender to identify _all synchronization units with updates since a given timestamp_ and pull as many changes in one go as possible. Note that the Receiver will need to be authenticated if any information (`listener_rev`) is accepted from it.
...
8. **[Receiver] RETURN**
   ...
   This will definitely lead to errors in some situations, but may work in the majority of the cases _if the operation reducers are fault-tolerant_ (which should be a goal anyway.) In that sense, it’s an interesting exercise to see how flexible for reordering the operations can be made.
