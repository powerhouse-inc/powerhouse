...
export const ShareMenuItem: React.FC<ShareMenuItemProps> = ({ label, url }) => {
  ...
  return (
    <div className="flex flex-col gap-1">
      <span className="text-xs text-gray-500 dark:text-slate-100">{label}</span>
      <div className="flex items-center gap-2">
        ...
        >
          <input
            ...
            readOnly
            value={url}
            className="w-[300px] truncate rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs text-gray-800 underline outline-none placeholder:text-gray-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-50"
          />
          {showTooltip && (
