// Renders React (RainbowKitProvider + wagmi hooks), so RSC hosts must treat this
// as a client module even though the descriptor entry reaches it lazily.
"use client";
...
export function createRainbowAdapter(
  ...
  return { Provider, useController: () => controller };
}
