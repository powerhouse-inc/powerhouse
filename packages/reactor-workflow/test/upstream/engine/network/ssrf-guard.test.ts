// Vendored from activepieces/activepieces@0.91.0 packages/server/engine/test/network/ssrf-guard.test.ts. MIT; see ../../../../../pieces-framework/LICENSE.
// Generated by packages/pieces-framework/scripts/sync-upstream.mts — do not edit by hand.
import dns from "node:dns";
import type { Server } from "node:net";
import { createServer, Socket } from "node:net";
import { SSRFBlockedError } from "../../../upstream-adapters/shared.js";
import {
  afterAll,
  afterEach,
  beforeAll,
...
describe("ssrf-guard", () => {
  ...
  describe("dns.lookup hook", () => {
    it("rejects hostname resolving to loopback via promises api", async () => {
      ssrfGuard.install({ enabled: true, allowList: [] });
      await expect(guardedDns.promises.lookup("localhost")).rejects.toBeInstanceOf(
        SSRFBlockedError,
      );
    });
    ...
    it("rejects hostname resolving to loopback via callback api", async () => {
      ...
      const err = await new Promise<unknown>((resolve) => {
        guardedDns.lookup("localhost", (e) => resolve(e));
      });
    ...
    it("honours allowList for normally blocked IP", async () => {
      ...
      const result = await guardedDns.promises.lookup("localhost", { family: 4 });
  ...
  describe("dns.lookup hook — multi-A-record coverage", () => {
    ...
    it("promises api: blocks when caller omits { all: true } but one A record is private", async () => {
      vi.spyOn(dns.promises, "lookup").mockResolvedValue(
        publicThenPrivate as unknown as dns.LookupAddress,
      );
      ssrfGuard.install({ enabled: true, allowList: [] });
    ...
    it("promises api: returns single-entry shape when caller omits { all: true } and all records are public", async () => {
      vi.spyOn(dns.promises, "lookup").mockResolvedValue(
        allPublic as unknown as dns.LookupAddress,
      );
      ssrfGuard.install({ enabled: true, allowList: [] });
      const result = await guardedDns.promises.lookup("public.example.test");
    ...
    it("callback api: passes first public entry when caller omits { all: true }", async () => {
      ...
      const result = await new Promise<{
        ...
        guardedDns.lookup("public.example.test", (err, address, family) =>
    ...
    it("callback api: passes full array when caller passes { all: true }", async () => {
      ...
      ssrfGuard.install({ enabled: true, allowList: [] });
      const result = await new Promise<{ err: unknown; address: unknown }>(
        (resolve) => {
          guardedDns.lookup("public.example.test", { all: true }, (err, address) =>
            resolve({ err, address }),
          );
        },
      );
  ...
  describe("net.Socket.connect hook", () => {
    ...
    // Not applicable here: our worker reaches its host over IPC, so the guard has no loopback-port allow-list
    it.skip("allows loopback target on a whitelisted port (engine↔worker RPC)", async () => {
      ssrfGuard.install({
        enabled: true,
        allowList: [],
        allowedLoopbackPorts: [publicPort],
      });
    ...
    it("blocks loopback connection to non-whitelisted ports", async () => {
      ssrfGuard.install({
        enabled: true,
        allowList: [],
        allowedLoopbackPorts: [publicPort + 1],
      });
      const result = await connectOnce({ host: "127.0.0.1", port: publicPort });
      expect(result.connected).toBe(false);
      expect(result.error).toBeInstanceOf(SSRFBlockedError);
  ...
  describe("disabled guard", () => {
    // Not applicable here: our guard is sealed once installed and cannot be uninstalled
    it.skip("leaves dns.lookup untouched", async () => {
      ssrfGuard.install({ enabled: false });
      expect(ssrfGuard.isEnabled()).toBe(false);
      const result = await guardedDns.promises.lookup("localhost");
      expect(result.address).toMatch(/127\.0\.0\.1|::1/);
    });

    // Not applicable here: our guard is sealed once installed and cannot be uninstalled
    it.skip("leaves net.Socket.connect untouched", async () => {
      ssrfGuard.install({ enabled: false });
