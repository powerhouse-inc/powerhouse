// Vendored from activepieces/activepieces@0.91.0 packages/server/engine/test/variables/props-validator.test.ts. MIT; see ../../../../../pieces-framework/LICENSE.
...
describe("Property Validation", () => {
  describe("required properties", () => {
    ...
    // Deliberate divergence: unparseable JSON or OBJECT text reaches the piece instead of failing validation
    it.fails("should validate required json property", async () => {
      const props = {
        json: Property.Json({
          displayName: "JSON",
          required: true,
      ...
      const { errors: validJsonStringErrors } =
        await propsProcessor.applyProcessorsAndValidators(
          { json: '{"key": "value"}' },
          props,
          PieceAuth.None(),
      ...
      const { errors: validArrayStringErrors } =
        await propsProcessor.applyProcessorsAndValidators(
          { json: "[1, 2, 3]" },
          props,
          PieceAuth.None(),
      ...
      const { errors: invalidJsonErrors } =
        await propsProcessor.applyProcessorsAndValidators(
      ...
      const { errors: emptyStringErrors } =
        await propsProcessor.applyProcessorsAndValidators(
          { json: "" },
          props,
          PieceAuth.None(),
      ...
      const { errors: invalidTextErrors } =
        await propsProcessor.applyProcessorsAndValidators(
          { json: "asd" },
          props,
          PieceAuth.None(),
    ...
    // Deliberate divergence: unparseable JSON or OBJECT text reaches the piece instead of failing validation
    it.fails("should validate optional json property with invalid value", async () => {
      const props = {
        json: Property.Json({
          displayName: "JSON",
          required: false,
      ...
      const { errors: validUndefinedErrors } =
        await propsProcessor.applyProcessorsAndValidators(
          { json: undefined },
          props,
          PieceAuth.None(),
    ...
    // Deliberate divergence: unparseable JSON or OBJECT text reaches the piece instead of failing validation
    it.fails("should validate required object property", async () => {
      const props = {
  ...
  describe("optional properties", () => {
    ...
    // Known divergence (README: optional props set to null): ours passes null through; propsProcessor maps it to undefined
    it.fails("should convert null to undefined for unset optional properties", async () => {
      const props = {
  ...
  describe("type validation", () => {
    // Deliberate divergence: unparseable JSON or OBJECT text reaches the piece instead of failing validation
    it.fails("should validate property types", async () => {
      const props = {
  ...
  describe("multi-select dropdown processing", () => {
    ...
    // Deliberate divergence: a required prop absent from the input fails validation
    it.fails.each([
    ...
    // Known divergence (README: optional props set to null): ours passes null through; propsProcessor maps it to undefined
    it.fails("should map nil to undefined on optional props, leaving required-ness to validation", async () => {
      const { processedInput, errors } =
        await propsProcessor.applyProcessorsAndValidators(
          {
