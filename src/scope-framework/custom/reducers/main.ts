...
export const reducer: ScopeFrameworkMainOperations = {
    setRootPathOperation(state, action) {
        ...
        if (!action.input.newRootPath.match(rootPathPattern)) {
            throw new Error(`Invalid root path format ${action.input.newRootPath}. Use one or more capital letters: 'A', 'D', 'AQ', ...`);
        }

        const findString = state.data.rootPath + '.', 
            newString = action.input.newRootPath + '.';

        state.data.rootPath = action.input.newRootPath;
        state.data.elements = state.data.elements.map(e => ({
            ...e,
    ...
    addElementOperation(state, action) {
        ...
        if (rootSegment(newElement.path) != state.data.rootPath) {
            throw new Error(`Cannot add element with root segment ${rootSegment(newElement.path)} to document with root path ${state.data.rootPath}`);
        }
        
        let inserted = false, nextElement = null;
        while (nextElement = state.data.elements.shift()) {
            ...
            if (!inserted && newElement.path == nextElement.path) {
                throw new Error(`New element's path ${newElement.path} already exists.`);
            }
    ...
    removeElementOperation(state, action) {
        const element = state.data.elements.filter(e => e.id == action.input.id)[0];
        if (element) {
            state.data.elements = state.data.elements.filter(e => !e.path.startsWith(element.path));
        }
    },
