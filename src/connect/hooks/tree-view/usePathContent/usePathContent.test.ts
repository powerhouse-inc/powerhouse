...
import { treeItems, driveItem } from '../mocks';
...
describe('TreeView hooks', () => {
    describe('usePathContent', () => {
        ...
        it('should return nodes at the root level of the filterPath', () => {
            ...
            const folderNodes = renderHook(() => usePathContent('drive/folder1/folder1.2'));
            ...
            expect(folderNodes.result.current[0].id).toEqual('drive/folder1/folder1.2/folder1.2.1');
        });

        it("should return an empty array when there's no results for the filterPath", () => {
            const { result } = renderHook(() => usePathContent('/a/path/that/does/not/exist'));
