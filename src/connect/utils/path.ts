...
export const isRootPath = (path: string, filterPath = '') => {
    ...
    if (filterPath.length > 0) {
        if (filterPath.charAt(filterPath.length - 1) !== '/') {
 ...
 */
export const isSubPath = (path: string, filterPath = '', includeRootPath = false) => {
    if (filterPath === '' || filterPath === '/') return true;

    let normalizedFilterPath = filterPath;

    if (normalizedFilterPath.charAt(normalizedFilterPath.length - 1) === '/') {
        normalizedFilterPath = normalizedFilterPath.substring(0, normalizedFilterPath.length - 1);
    }

    if (normalizedFilterPath.charAt(0) === '/') {
        normalizedFilterPath = normalizedFilterPath.substring(1);
    }

    const regexpStr = includeRootPath ? `^/?${normalizedFilterPath}(/.*)?$` : `^/?${normalizedFilterPath}/.+$`;
...
export const filterItemsByPath = <T extends { path: string }>(
    items: Array<T>,
    filterPath = '',
    customFilter = (item: T) => true,
): Array<T> => items.filter(item => isRootPath(item.path, filterPath) && customFilter(item));
