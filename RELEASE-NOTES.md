...
The reactor's own user is read on every ask, so a login takes effect immediately. A host policy caches on (address, key) only. `createRenownTrustPolicy` retries a credential it has not found yet: the job is deferred until `missingCredentialWindowMs` (default 5 minutes) after the first miss, and only then is the write refused. A trust policy error carrying `retryAfterMs` defers the job without using its retries, holding later jobs on the same stream behind it, for up to `maxAdmissionDeferralMs` (default 6 minutes); `JobInfo.deferral` reports the wait. A switchboard refuses to boot with `REACTOR_WORKERS` above 0, `REACTOR_AUTH_ENFORCEMENT` on and `RENOWN_SOURCE=self`, because its pooled workers would have no trust policy. `RemoteDocumentController` resolves a slug to the document id before it signs. `withSignaturePolicy(document, "v2-required", { id })` ignores `id` and derives the id.
