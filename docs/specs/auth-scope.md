...
What remains is the read path — stage 7 below. Every shipped stage evaluates operations; reads still run through the stage-1 interim gate, which evaluates the bare policy with no groups map, no condition context, and no document projection, and bypasses the decision model entirely. Stage 4's step 8 named this work and deferred it.
...
1. **Route reads through the registered decision model.** The read filter — `canReadScope` and `filterReadableScopes` in `packages/reactor/src/client/util.ts`, applied by the reactor client's read functions and by the sync manager per remote — calls the bare `decide(auth, subject, request)` today: no groups map, no condition context, no document projection. It must instead build the registered model at the stream heads, the way `decideAtHead` already does for admission, and evaluate a `read` request per scope. With that, `{ group }` and conditional read grants apply for the first time. A read's condition context carries the scope's own state and no action input, so a condition on `action.input.*` never holds for a read. The `auth` and `document` scopes stay exempt (see Reads); `ALWAYS_READABLE_SCOPES` already encodes the exemption.

2. **Serve policy-named groups to the policy's audience.** A replica must fold a group's membership to evaluate auth with it, so a group document named by a grant is served to any subject the naming document's policy serves, regardless of the group's own read grants (see Reads and Synchronization). Concretely: when the read filter or the sync manager evaluates a group document's `global` scope, the subject is readable if it is readable under any document holding a `group_references` row that names this group. `getGroupReferencers` on the operation index is the lookup.

3. **Skip denied operations in `replayDocument`.** The reactor's own rebuild path applies nothing for an operation stored denied, but `replayDocument` in `shared/document-model` still applies it, so a client replaying exported history diverges from the state every reactor holds. It must skip denied operations exactly as the write cache and the positional walk do.

4. **Serve deletion-boundary state.** The read surface hides a deleted document outright. Deletion is positional (stage 3), so the correct read is the state at the deletion boundary: every scope folded through the operations that sort before the delete, nothing after.

5. **Close the versioned-documents gap in the positional walk.** `foldEvaluatedScope` (stage 6) resolves one reducer version for the entire walk — the base document's stamped version, which is the latest one, because a domain-scope rebuild leaves the document scope unbounded. A history that crosses an `UPGRADE_DOCUMENT` boundary therefore folds pre-upgrade operations with the post-upgrade reducer, unlike the write cache's own upgrade-aware rebuild. `UPGRADE_DOCUMENT` is also not a deciding action on the document projection, so a version change that flips a condition's outcome triggers no re-evaluation. Either the walk becomes upgrade-aware — switching reducers at upgrade positions, as the cache does — or `authConditions` documents a single-version limitation and `preflight:auth` checks it. Until one of those ships, `authConditions` must not be enabled on a fleet holding multi-version documents.

Exit tests: the worked toll statement's read grants hold — the RTO's `match` grant serves them their own statement and nobody else's, and a group-gated read grant follows a membership change with no policy write; a client replaying exported history containing a denied operation reproduces the state the server holds; and a deleted document reads as the state at its deletion boundary rather than vanishing.
...
8. **Filter reads and sync against the same model.** This is where stage 3's two read-side limits close: `replayDocument` applying denied operations, and reads hiding a deleted document rather than serving the state at the deletion boundary. _Deferred: this step did not ship with stage 4; it is stage 7._
