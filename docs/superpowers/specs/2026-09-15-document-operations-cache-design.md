...
- Alongside each entry the cache keeps the `next` function of the last `PagedResults` and an `AbortController`, in a separate private map. These are not part of the snapshot.
- `loadOperations` calls `client.getOperations(documentId, { scopes: [scope] }, undefined, { cursor: "", limit }, signal)`. `loadMoreOperations` calls the stored `next()`. A resolved page sets `status: "success"`, appends `results`, sets `hasNextPage: !!page.next`, and copies `totalCount`. A rejected page sets `status: "error"` with the reason and keeps the operations loaded so far. An aborted request changes nothing.
...
- Because pages arrive oldest first and the view is newest first, the component asks for the next page itself: an effect calls `onLoadNextPage` whenever `hasNextPage` is true and `isLoading` is false. The loaded operations render progressively, and a "Loading more operations…" line shows below the timeline while `isLoading` is true and some operations are already rendered.
